...
async function fetchHPScanners() {
    try {
        ...
        hpScanners = await response.json();
        displayHPScanners(hpScanners);
    } catch (error) {
...
async function fetchEpsonScanners() {
    try {
        ...
        epsonScanners = await response.json();
        displayEpsonScanners(epsonScanners);
    } catch (error) {
...
async function fetchRicohScanners() {
    try {
        ...
        ricohScanners = await response.json();
        displayRicohScanners(ricohScanners);
    } catch (error) {
...
async function fetchCanonScanners() {
    try {
        ...
        canonScanners = await response.json();
        displayCanonScanners(canonScanners);
    } catch (error) {
...
function displayHPScanners(products) {
    ...
    hpProductContainer.innerHTML = products.map((item, index) => createScannerHTML(item, index, 'hp')).join('');
}
...
function displayEpsonScanners(products) {
    ...
    epsonProductContainer.innerHTML = products.map((item, index) => createScannerHTML(item, index, 'epson')).join('');
}
...
function displayRicohScanners(products) {
    ...
    ricohProductContainer.innerHTML = products.map((item, index) => createScannerHTML(item, index, 'ricoh')).join('');
}
...
function displayCanonScanners(products) {
    ...
    canonProductContainer.innerHTML = products.map((item, index) => createScannerHTML(item, index, 'canon')).join('');
}

// Create HTML for a scanner
function createScannerHTML(item, index, brand) {
...
function addToCart(brand, index) {
    ...
    switch (brand) {
        case 'hp':
            productsArray = hpScanners;
            break;
        case 'epson':
            productsArray = epsonScanners;
            break;
        case 'ricoh':
            productsArray = ricohScanners;
            break;
        case 'canon':
            productsArray = canonScanners;
            break;
        default:
            return;
    }
